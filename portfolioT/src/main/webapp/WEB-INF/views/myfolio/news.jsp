			...
			$("#journalName option:selected").each(function(){
				...
				$.ajax({
				    ...
				    success: function(result){
							...
							//$("iframe").prop("src", "");
							//$("iframe").prop("src", this.journalLink);
						//})
				    },
				    error: function(){
...
<body>
	<div id="content">
		...
		<div id="box">
			...
			<div class="box-right">
				<iframe id="inner" src="${articles[0].href }"></iframe>
				<%-- <ul style="list-style: none;">
		<c:forEach items="${articles1 }" var="articles1" >
			<li class="news">
			<p class="journal"><a href="${articles1.journalLink }">${articles1.journal }</a></p>
				<h3 class="title">
					<a href="${articles1.href }" class="link">${articles1.title}</a>
				</h3><hr/>
				<c:forEach items="${articles1.sumArticle }" var="articles.sumAtricle" varStatus="status">
					<p class="text">${articles1.sumArticle[status.index] }</p>
				</c:forEach>
				<div class="linkBtn"><a href="${articles1.href }">원문 보기</a></div>
			</li>
		</c:forEach>
		</ul> --%>
			</div>
